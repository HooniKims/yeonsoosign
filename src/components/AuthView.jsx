    ...
    <div className="field">
      ...
      <div className="school-search-shell">
        ...
        {searchState === "idle" && query.trim().length > 0 && query.trim().length < 2 ? (
          <p className="micro-copy">학교명 두 글자 이상부터 검색됩니다.</p>
        ) : null}
        {searchState === "error" ? (
          <p className="inline-feedback inline-feedback-error">학교 검색에 실패했습니다. 잠시 후 다시 시도해 주세요.</p>
        ) : null}
...
export default function AuthView({
  ...
  }, [deferredSchoolQuery, mode, selectedSchool]);
  ...
  async function handleSubmit(event) {
    ...
    if (!selectedSchool) {
      throw new Error("학교를 검색 결과에서 선택해야 가입할 수 있습니다.");
    }
  ...
  const title = intent === "cloud_setup" ? "학교 설정 전용 로그인" : "관리자 로그인";
        ...
        <div className="auth-card surface-card">
          <div className="auth-copy">
            ...
            <p className="muted-copy">
              {APP_NAME} 관리자 계정은 이메일 또는 Google 계정으로 가입할 수 있습니다.
            </p>
          </div>
          ...
          {!hasFirebaseConfig ? (
            <article className="notice-card notice-card-warm">
              ...
              <p>배포 환경에 Firebase 웹 앱 설정값을 추가해야 관리자 인증을 사용할 수 있습니다.</p>
            </article>
          ) : null}
